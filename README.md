...
Click the **Conclusión** button in the side panel toolbar to get a two-part summary of the current session's results:

| Part | Detail |
|---|---|
| **Instant local tally** | Verdict counts (Verdadera / Falsa / Dudosa / No verificable) and average confidence — computed immediately from the claims loaded in the panel, with no API call. |
| **AI-written conclusion** | A 3–6 sentence Spanish-language paragraph synthesizing the session's overall reliability — generated by your configured provider on demand. |

### Notes

- The AI conclusion requires an API key (skipped automatically for Ollama and other keyless providers).
- The call uses the same provider and model configured in **Configuración** and consumes a small number of tokens.
- The local tally and AI conclusion reset when you click **Limpiar** or switch to a different tab.
