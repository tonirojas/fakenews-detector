...
function parseAnalysisResult(raw, fallbackReason) {
  ...
  // The pattern matches from <think> to the first </think> OR end-of-string,
  // covering both complete blocks and unterminated opening tags in one pass.
  let withoutThink = raw.replace(/<think>(?:(?!<\/think>)[\s\S])*?(?:<\/think>|$)/gi, "");
