...
// ---------------------------------------------------------------------------
// Request timeout — applies to every provider fetch
// ---------------------------------------------------------------------------
const REQUEST_TIMEOUT_MS = 120000; // 120 seconds (generous for slow reasoning models)

/**
 * Wraps fetch() with a per-call AbortController timeout.
 * On timeout, throws a user-visible Spanish error.
 * Network errors (non-AbortError) are re-thrown as-is.
 *
 * @param {string} url
 * @param {RequestInit} options  Merged with the abort signal; do not include `signal` here.
...
async function fetchWithTimeout(url, options, timeoutMs) {
  ...
  try {
    ...
    if (err.name === "AbortError") {
      throw new Error(
        "La IA no respondió a tiempo (límite de " +
          Math.round(timeoutMs / 1000) +
          " s). Prueba un modelo más rápido."
      );
    }
    throw err;
  } finally {
...
async function callOpenAI({ apiKey, model, text, baseUrl, requiresKey, system, jsonMode = true }) {
  ...
  if (jsonMode) {
    ...
    let response = await fetchWithTimeout(url, { method: "POST", headers, body: bodyWith }, REQUEST_TIMEOUT_MS);
    ...
    if (!response.ok && response.status >= 400 && response.status < 500) {
      ...
      if (/response_format|json/i.test(errText)) {
        ...
        response = await fetchWithTimeout(url, { method: "POST", headers, body: bodyWithout }, REQUEST_TIMEOUT_MS);
        if (!response.ok) {
          const excerpt = (await response.text()).slice(0, 200);
...
export async function factCheck(settings, text, context) {
  ...
  // Defense in depth: validate the effective base at request time (not only at
  // options-save) so a directly-written storage value cannot send the API key
  // to a non-https / attacker host. Mirrors the Whisper + webhook paths.
  const baseErr = validateBaseUrl(base);
  if (baseErr) throw new Error(baseErr);
...
export async function summarizeSession(settings, claims) {
  ...
  const base     = effectiveBase(settings);
  // Defense in depth: validate the effective base at request time.
  const baseErr = validateBaseUrl(base);
  if (baseErr) throw new Error(baseErr);
  ...
  const stripped = stripThinkBlocks(raw).replace(/<\/think>/gi, "").trim();

  // Detect reasoning-only responses: the model emitted a <think> block but no
  // conclusion text. This happens with MiniMax M2.x and similar models when the
  // token budget runs out inside the reasoning block or the prompt confuses the
  // model into thinking-only output.
  if (!stripped) {
    if (!raw || !raw.trim()) {
      throw new Error("El modelo no devolvió contenido.");
    }
    throw new Error(
      "El modelo devolvió solo razonamiento, sin conclusión final. " +
        "Prueba un modelo más rápido, p. ej. DeepSeek-chat, GLM-4.5-Air o Gemini Flash."
    );
  }
  return stripped;
}
...
export async function transcribe({ settings, base64Audio, mimeType }) {
  ...
  if (useWhisper) {
    ...
    const response = await fetchWithTimeout(`${sttBase}/audio/transcriptions`, {
      method: "POST",
  ...
  if (provider.protocol === "gemini") {
    const base = effectiveBase(settings);
    // Defense in depth: validate the effective base before sending audio + key.
    const baseErr = validateBaseUrl(base);
    if (baseErr) throw new Error(baseErr);
